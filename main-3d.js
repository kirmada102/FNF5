...
function createIstanbulStreet() {
  ...
  // Strong yellow street lamps along edges + blossom LED trees next to each lamp.
  ...
  for (let z = -60; z <= 60; z += 10) {
    for (const side of [-1, 1]) {
      ...
      // Tree is placed beside lamp on main Galata street edge (not inside side alleys).
      const treeX = pole.position.x + side * 2.2;
      const treeZ = z + (Math.random() - 0.5) * 1.6;
      const treeScale = 0.82 + Math.random() * 0.18;
      createLitBlossomTree(istanbulGroup, treeX, treeZ, treeScale, true);
    }
  }
