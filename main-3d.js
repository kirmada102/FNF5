...
function createCat(x, z, follow = false) {
  ...
  const bodyMat = new THREE.MeshStandardMaterial({ color: palette.body, roughness: 0.8 });
  const stripeMat = new THREE.MeshStandardMaterial({ color: palette.stripe, roughness: 0.8 });
  const pawMat = new THREE.MeshStandardMaterial({ color: palette.paw, roughness: 0.8 });
  const eyeMat = new THREE.MeshStandardMaterial({ color: palette.eye, emissive: palette.eye, emissiveIntensity: 0.4 });
  const noseMat = new THREE.MeshStandardMaterial({ color: palette.nose });

  const body = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.7, 0.45), bodyMat);
  body.position.y = 0.45;
  group.add(body);

  // stripes
  for (let i = -1; i <= 1; i++) {
    const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.08, 0.46), stripeMat);
    stripe.position.set(-0.4 + i * 0.4, 0.55, 0);
    group.add(stripe);
  }
  ...
  const earGeo = new THREE.BoxGeometry(0.16, 0.16, 0.12);
  const leftEar = new THREE.Mesh(earGeo, bodyMat);
  const rightEar = new THREE.Mesh(earGeo, bodyMat);
  leftEar.position.set(1.05, 0.95, -0.16);
  rightEar.position.set(1.05, 0.95, 0.16);
  group.add(leftEar, rightEar);

  const eyeGeo = new THREE.BoxGeometry(0.14, 0.12, 0.04);
  const leftEye = new THREE.Mesh(eyeGeo, eyeMat);
  const rightEye = new THREE.Mesh(eyeGeo, eyeMat);
  leftEye.position.set(1.12, 0.68, -0.12);
  rightEye.position.set(1.12, 0.68, 0.12);
  group.add(leftEye, rightEye);

  const nose = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.08, 0.08), noseMat);
  nose.position.set(1.16, 0.58, 0);
  group.add(nose);
  ...
  // tail
  const tail = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.18, 1.0), bodyMat);
...
function createIstanbulStreet() {
  ...
  // Cars on the main street
  for (let i = 0; i < 6; i++) {
    const car = createCar([0x3e7fff, 0xff5a5a, 0x4ac16d, 0xf0a000][i % 4]);
    car.position.set((Math.random() - 0.5) * (ISTANBUL.roadWidth - 4), 0.05, (Math.random() - 0.5) * (ISTANBUL.length - 10));
    car.rotation.y = Math.random() < 0.5 ? 0 : Math.PI;
    istanbulGroup.add(car);
  }

  // Strong yellow street lamps along edges
  for (let z = -60; z <= 60; z += 10) {
    for (const side of [-1, 1]) {
      ...
      const lamp = new THREE.PointLight(0xffe27a, 1.5, 30, 2);
...
function spawnFinalBoyFollower() {
  finalBoy = createBoy();
  finalBoy.position.copy(player.position).add(new THREE.Vector3(1.2, 0, 0));
  scene.add(finalBoy);
}

function updateFinalBoy(delta) {
  if (!finalBoy) return;
  const target = player.position.clone().add(new THREE.Vector3(-1, 0, -1));
  moveToward(finalBoy, target, 2.2, delta);
}

function showFinalLetter() {
  finalLetterShown = true;
  finalPending = false;
  if (!finalBoy) spawnFinalBoyFollower();
  if (pointerLocked) document.exitPointerLock();
  overlay.style.display = 'flex';
  overlayContent.className = 'overlay-card';
...
- yours geet`;
  ...
  overlayContent.innerHTML = `
    <div style="background: rgba(255,255,255,0.15); border: 2px solid rgba(255,255,255,0.25); padding: 18px; border-radius: 12px; box-shadow: inset 0 0 12px rgba(255,255,255,0.35);">
      ...
      <button id="finalReplay" style="margin-top:14px;padding:10px 18px;border:none;border-radius:8px;background:#ff3b7a;color:#fff;font-weight:bold;cursor:pointer;">Replay</button>
    </div>
  `;
  ...
  document.getElementById('finalReplay').onclick = () => {
    overlay.style.display = 'none';
    resetToForest();
    startStage();
    renderer.domElement.requestPointerLock();
