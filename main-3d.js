...
const LEVEL1_SKY = new THREE.Color(0xf0b9c2);
const BEACH = {
  centerZ: -120,
  width: 140,
  depth: 80,
...
function createBeachScene() {
  ...
  const seaMat = new THREE.MeshStandardMaterial({ color: 0x5aa9c9, transparent: true, opacity: 0.85 });
  ...
  const sea = new THREE.Mesh(new THREE.PlaneGeometry(BEACH.width * 1.2, BEACH.seaDepth), seaMat);
  ...
  stick.position.set(-1.6, 0.3, BEACH.centerZ + 8);
  stick.rotation.z = -0.6;
  beachStick = stick;
  beachGroup.add(girl, stick);
...
function showIntroLetter() {
  ...
  scene.fog = new THREE.Fog(0xf2c0c8, 60, 220);
...
function resetToForest() {
  ...
  scene.background = LEVEL1_SKY;
  scene.fog = new THREE.Fog(0xf2c0c8, 90, 300);
...
function updateBeachIntro(delta, timeSeconds) {
  if (introLetterActive || beachIntroActive) {
