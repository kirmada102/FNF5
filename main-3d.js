...
function createRose() {
  ...
  const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.5, 6), new THREE.MeshStandardMaterial({ color: 0x3f8f3f }));
  stem.position.y = 0.25;
  stem.castShadow = true;
  rose.add(stem);

  const bloom = new THREE.Mesh(new THREE.IcosahedronGeometry(0.12, 0), new THREE.MeshStandardMaterial({ color: 0xe9345b }));
  bloom.position.y = 0.55;
  bloom.castShadow = true;
  rose.add(bloom);

  const leaf = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.02, 0.05), new THREE.MeshStandardMaterial({ color: 0x4f9f4a }));
