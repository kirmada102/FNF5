...
function createIstanbulStreet() {
  ...
  // Pink blossom trees with LED lights around the city edges.
  cityBlossomTrees.length = 0;
  const blossomSpots = [
    [ISTANBUL.roadWidth / 2 + 14, -52],
    [ISTANBUL.roadWidth / 2 + 22, -34],
    [ISTANBUL.roadWidth / 2 + 16, -14],
    [ISTANBUL.roadWidth / 2 + 24, 8],
    [ISTANBUL.roadWidth / 2 + 18, 28],
    [ISTANBUL.roadWidth / 2 + 22, 50],
    [-ISTANBUL.roadWidth / 2 - 14, -48],
    [-ISTANBUL.roadWidth / 2 - 20, -26],
    [-ISTANBUL.roadWidth / 2 - 16, -4],
    [-ISTANBUL.roadWidth / 2 - 24, 18],
    [-ISTANBUL.roadWidth / 2 - 18, 38],
    [-ISTANBUL.roadWidth / 2 - 22, 56]
  ];
  blossomSpots.forEach((p, idx) => {
    const scale = 0.85 + Math.random() * 0.25;
    createLitBlossomTree(istanbulGroup, p[0], p[1], scale, idx % 3 === 0);
  });
...
function updatePetals(delta, timeSeconds) {
  const petalSourceTrees = inIstanbul && cityBlossomTrees.length > 0 ? cityBlossomTrees : trees;
  if (petalSourceTrees.length > 0 && petals.length < 260 && Math.random() < 0.5) {
    const tree = petalSourceTrees[Math.floor(Math.random() * petalSourceTrees.length)];
